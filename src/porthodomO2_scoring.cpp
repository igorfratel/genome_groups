...
static int clustering_value(protein_info_t prot_g1_1, protein_info_t prot_g1_2, protein_info_t prot_g2_1, protein_info_t prot_g2_2, ProteinCollection &clusters,
                            double stringency) {
  ...
  double similarity1 = clusters.get_similarity(prot_g1_1.pid, prot_g2_1.pid);
  double similarity2 = clusters.get_similarity(prot_g1_2.pid, prot_g2_2.pid);
  double result = (similarity1 + similarity2)/2;
...
static std::vector<std::vector<int> > fill_assignment_matrix(GenomicNeighborhood g1, GenomicNeighborhood g2,
    ...
    matrix.resize(g1.protein_count() - 1); //Number of pairs of proteins in g1
    for (unsigned int k = 0; k < matrix.size(); k++)
        matrix[k].resize(g2.protein_count()); //Number of pairs of proteins in g2
